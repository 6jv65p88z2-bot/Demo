...
volatile bool dma_trans_complete_flag = false;
volatile uint32_t adc_complete_flag = 0;
...
extern void tmr7_global_callback(void);
extern void power_off_callback(void);
...
/**
  * @brief  this function handles ADC1 2 3 handler.
  * @param  none
  * @retval none
  */
void ADC1_2_3_IRQHandler(void)
{
  /* add user code begin ADC1_2_3_IRQ 0 */
	if(adc_interrupt_flag_get(ADC1, ADC_OCCO_FLAG) != RESET)
	{
		adc_flag_clear(ADC1, ADC_OCCO_FLAG);
	}
  /* add user code end ADC1_2_3_IRQ 0 */
  /* add user code begin ADC1_2_3_IRQ 1 */

  /* add user code end ADC1_2_3_IRQ 1 */
}
...
volatile uint16_t UI_fresh_flag = 0;
/**
